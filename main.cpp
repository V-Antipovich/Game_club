...
int main(int argc, char *argv[]) {
    try {
        ...
        GamerClubEnv env(parser_file.tablesNum, parser_file.costPerHour, parser_file.startWorkTime,
                         parser_file.endWorkTime, parser_file.inputEventsQueue);
        env.HandleInputEvents();
        env.Print();
    } catch (const ParserConsoleException &pce) {
        std::cout
                << "Чтобы программа корректно выполнилась, ей на вход необходимо предоставить один аргумент - путь до файла\n"
